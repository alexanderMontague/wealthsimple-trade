...
async function getCurrPortfolioValue(portfolio, userBaseCurrency) {
    ...
    return Object.keys(portfolio).reduce((currentPortfolioValue, coin) => {
        return (
            currentPortfolioValue + currCoinPrices[coin] * currCoinAmounts[coin]
        )
    }, 0)
}
...
async function updatePortfolio(portfolio, userBaseCurrency = 'CAD') {
    ...
    // get current prices of all coins in portfolio
    const currCoinInfo = await getMultipleCoinInfo(
        Object.keys(portfolioCoins),
        userBaseCurrency
    )
    ...
    for (const coin in portfolio) {
        if (coin !== 'meta') {
            ...
            portfolio[coin].change24HourDollar =
                currCoinAmounts[coin] *
                currCoinInfo[coin][userBaseCurrency].CHANGE24HOUR
            portfolio[coin].change24HourPercent =
                currCoinAmounts[coin] *
                currCoinInfo[coin][userBaseCurrency].CHANGEPCT24HOUR
        }
    }
    ...
    portfolio.meta.currentTotalValue = Object.keys(portfolioCoins).reduce(
        (currentPortfolioValue, coin) => {
            return (
                ...
                currCoinPrices[coin] * currCoinAmounts[coin]
            )
        },
        0
    )
