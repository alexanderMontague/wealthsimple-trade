...
const tradeState = (prevState = initialState, { type, payload }) => {
    switch (type) {
        ...
        case 'ADD_TO_PORTFOLIO':
            ...
            const prevPortfolio = JSON.parse(
                JSON.stringify(prevState.portfolio)
            ) // creates deep immutable copy
            if (!prevPortfolio[ticker]) {
                ...
                const historicTotalValue = getHistoricPortfolioValue(
                    updatedPortfolio
                )
            ...
            newCoinAsset.totalCoinAmount =
                Number(newCoinAsset.totalCoinAmount) +
                Number(details.amountBought)
            // add a new lot
